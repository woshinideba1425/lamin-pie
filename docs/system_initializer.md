...
| **2. BSP 初始化（Board Support Package）** | ✅      | 与具体板子有关的初始化：pinmux、IO remap、低功耗配置等 |
| **3. 驱动初始化**                          | ✅      | 各类驱动，如 I2C、SPI、UART、LCD、Sensor 等   |
| **4. 中间件 / 协议栈初始化**                   | ⭕️可选   | 如 FATFS、LVGL、LwIP、BLE Stack 等框架性组件 |
| **5. 系统服务初始化**                        | ✅      | 如任务调度器（RTOS）、定时器服务、日志系统、事件管理等      |
