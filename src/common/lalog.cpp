...
void LogPrintf(int level,const char*file,const char*func,int line,const char*format,...) {
    ...
    int len1 = snprintf(msgBoddy, kMaxMessageSize,
        "%010lld.%06lld \033[0;32m[%s]\033[0;34m %s:%d %s",
        (long long)ts.tv_sec, (long long)(ts.tv_nsec / 1000),
        tag.c_str(), func, line, colors[level]);
...
void LogDump(int level,const char*tag,const char*func,int line,const char*label,const unsigned char*data,int len) {
    char buff[128];
    sprintf(buff,"%s[%d]",label,len);
#if __cplusplus >= 202002L
    // 在C++20下避免模板实参推导问题，直接拼装并输出
    const char*colors[]= {"\033[0m","\033[1m","\033[0;32m","\033[0;36m","\033[1;31m","\033[0;31m"};
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC,&ts);
    std::ostringstream oss;
    oss << std::setw(10) << std::setfill('0') << (long long)ts.tv_sec << "." << std::setw(6) << (long long)(ts.tv_nsec/1000);
    oss << " \033[0;32m[" << (tag?tag:"") << "]\033[0;34m " << (func?func:"") << ":" << line << " " << colors[level];
    oss << buff;
    for(int i=0; i<len; i++) {
    ...
    entry += "\033[0m\n";
#if defined(ASYNC_LOG)&&ASYNC_LOG
    dbgMessages.push(entry);
#else
    printf("%s", entry.c_str());
#endif
#else
    lalog::LogMessage log(tag,line,func,level);
...
int QueryModuleLevel(const std::string& fileName) {
    const std::string tag = splitFileName(fileName);
    auto it = sModules.find(tag);
    return (it == sModules.end()) ? sLogLevel : it->second;
}
