...
#if !LAMINPIE_LVGL_HELPER_ENABLE_DEBUG_LOG
...
lv_align_t toLvAlign(const StyleAlignType &type)
{
    ...
    default:
        LAMINPIE_LVGL_LOG_ERROR("Invalid align type: %d, use default.", type);
...
lv_flex_flow_t toLvFlexFlow(const StyleLayoutFlex::FlowType &flow)
{
    ...
    default:
        LAMINPIE_LVGL_LOG_ERROR("Invalid flow type: %d, use default.", flow);
        return LV_FLEX_FLOW_ROW;
    }
}
...
lv_flex_align_t toLvFlexAlign(const StyleLayoutFlex::AlignType &align)
{
    ...
    default:
        LAMINPIE_LVGL_LOG_ERROR("Invalid align type: %d, use default.", align);
        return LV_FLEX_ALIGN_START;
    }
}
...
bool getLvInternalFontBySize(uint8_t size_px, const lv_font_t **font)
{
    ...
    if (temp_size != size_px) {
        LAMINPIE_LVGL_LOG_WARN("Font size(%d) not support, use the nearest size(%d)", temp_size, size_px);
    }
    ...
    default:
        ret = false;
        LAMINPIE_LVGL_LOG_ERROR("No internal font size(%d) found, use default instead", temp_size);
        break;
    }
...
lv_anim_path_cb_t getLvAnimPathCb(StyleAnimation::AnimationPathType type)
{
    ...
    LAMINPIE_LVGL_LOG_ERROR("Invalid animation path type(%d)", type);
