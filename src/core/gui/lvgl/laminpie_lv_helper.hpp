...
lv_anim_path_cb_t getLvAnimPathCb(laminpie::gui::StyleAnimation::AnimationPathType type);
} // namespace laminpie::gui
 ...
 */
using ESP_Brookesia_LvObj_t = laminpie::gui::LvObjSharedPtr;
using ESP_Brookesia_LvTimer_t = laminpie::gui::LvTimerSharedPtr;
using ESP_Brookesia_LvAnim_t = laminpie::gui::LvAnimSharedPtr;
...
{
    return laminpie::gui::getLvInternalFontBySize(size_px, font);
}
inline lv_color_t esp_brookesia_core_utils_get_random_color(void)
{
    return laminpie::gui::getLvRandomColor();
}
inline bool esp_brookesia_core_utils_check_obj_out_of_parent(lv_obj_t *obj)
{
    return laminpie::gui::checkLvObjOutOfParent(obj);
}
inline bool esp_brookesia_core_utils_check_event_code_valid(lv_event_code_t code)
{
    return laminpie::gui::checkLvEventCodeValid(code);
}
inline lv_indev_t *esp_brookesia_core_utils_get_input_dev(const lv_display_t *display, lv_indev_type_t type)
{
    return laminpie::gui::getLvInputDev(display, type);
}
inline lv_anim_path_cb_t esp_brookesia_core_utils_get_anim_path_cb(laminpie::gui::StyleAnimation::AnimationPathType type)
{
    return laminpie::gui::getLvAnimPathCb(type);
}
